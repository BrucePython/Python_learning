...
-- mysql是关系型数据库系统(RDBMS)的一套软件, 用这套软件创建和管理数据库;
-- C/s类型: rdbms客户端 发送sql(类似于http)命令 控制 rdbms服务器(包含mysql软件) 
-- SQL是结构化查询语言，是一种用来操作RDBMS的数据库语言
-- 数据库的操作
-- show create database/table 名字;  以代码的方式显示字段信息
-- desc 名字; 以表格的方式显示字段信息
    ...
    -- 创建数据库
    -- create database 数据库名 charset=utf8; 因为创建一个数据库的时候, 如果不指定编码, 则默认的是拉丁文
...
-- 数据表的操作
    ...
    create table students(
        ...
        high decimal(5,2),           -- 五位数字,其中两位是小数
        gender enum("男", "女", "中性", "保密") default "保密",
        cls_id int unsigned             -- unsigned: 不包括负数
    );
    ...
    alter table students change birthday birth date default "2000-01-01";   -- 如果之间已经有数据, 原数据不会自动修改默认值.
